...
- **タグっぽく見せる**
  - `#日記` (ハッシュの直後に文字): SNSのハッシュタグのように表示され、このデザインでは自動的に色がつくようになっています。
...
2. **有料記事の作成**: codoc ダッシュボードで記事を作成します。
3. **記事コードの取得**: 表示された「貼付けタグ」画面の一番下にある **「記事コード：xxxxxxx」** という短い英数字（例: `ntttO1nMAg`）だけをコピーします。  
    - **注**: 画面中央に出ている「JS+要素」や「要素のみ」などの長いコードを丸ごとコピーする必要はありません。
4. **日記に貼り付け**: 日記の Markdown 内で以下のように記述します。
...
> [!TIP]  
> どちらか一方のタグだけでも動作しますが、`codoc id="..."` で決済ボタンを表示し、`codoc-paywall` を使うと「ここから有料」という境界線が綺麗に表示されるのでセットで使うのがおすすめです。
