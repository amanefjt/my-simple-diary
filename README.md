...
*   **title**: サイト上で表示される大きなタイトル。
*   **date**: 日付（`2026-04-03` のように半角の数字とハイフンで書くだけでOKです）。
*   **draft**: `false` にすると公開されます。`true` だと下書き扱いになり表示されません。

---
### ✍️ マークダウンの書き方（クイックガイド）

日記の文章は「マークダウン」という、記号を使って装飾する書き方に対応しています。

#### 1. 改行
マークダウンでは、**「空の行を一行入れる」** ことで段落が分かれます。
文末で普通に1回改行しただけでは、サイト上では一行に繋がって表示されることがあります。
...
```markdown
> これは引用文です。
> 人の言葉やニュースなどを引用するときに使います。
```
※ 記号（`>`）の後に **必ず半角スペース** を入れるのがコツです！
